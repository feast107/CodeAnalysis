...
namespace Feast.CodeAnalysis.Scripting
{
    ...
    /// </summary>
    [Literal("Feast.CodeAnalysis.Scripting.Generates.Script")]
    public abstract partial class Script
    {
        ...
        internal static readonly MetadataReferenceProperties HostAssemblyReferenceProperties =
            (MetadataReferenceProperties)WithRecursiveAliases(
                MetadataReferenceProperties.Assembly.WithAliases(ImmutableArray.Create("<host>")), true);
            ...
            MetadataReference languageRuntimeReferenceOpt = null)
        {
            ...
            try
            {
                if (Previous == null)
                {
                    var corLib = CreateFromAssemblyInternal.Value(typeof(object).GetTypeInfo().Assembly) as MetadataReference;
                    ...
                    if (GlobalsType != null)
                    {
                        ...
                        if (HasMetadata.Value(globalsAssembly) is true)
                        {
                            references.Add(CreateFromAssemblyInternal3.Value(globalsAssembly, HostAssemblyReferenceProperties) as MetadataReference);
                        }
                    }
                ...
                foreach (var reference in Options.MetadataReferences)
                {
                    if (reference is UnresolvedMetadataReference unresolved)
                    {
                        ...
                        {
                            diagnostics.Add(CreateDiagnostic.Value(messageProvider,ERR_MetadataFileNotFound.Value(messageProvider), Location.None, unresolved.Reference) as Diagnostic);
                        }
                        else
                        {
    ...
    [Literal("Feast.CodeAnalysis.Scripting.Generates.Script_T")]
    public sealed class Script<T> : Script
    {
        ...
        private ImmutableArray<Func<object[], Task>> GetPrecedingExecutors(CancellationToken cancellationToken)
        {
            if (_lazyPrecedingExecutors.IsDefault)
            {
                ...
                object[] args = [_lazyPrecedingExecutors, preceding];
                Initialize.Value.Invoke(null, args);
                //InterlockedOperations.Initialize(ref _lazyPrecedingExecutors, preceding);
                _lazyPrecedingExecutors = (System.Collections.Immutable.ImmutableArray<System.Func<object[],System.Threading.Tasks.Task>>)args[0];
            }
...
[Literal("Feast.CodeAnalysis.Scripting.Generates.ScriptStateTaskExtensions")]
internal static class ScriptStateTaskExtensions
{
    ...
    internal static async Task<T> GetEvaluationResultAsync<T>(this Task<Feast.CodeAnalysis.Scripting.ScriptState<T>> task)
    {
