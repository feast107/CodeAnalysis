...
namespace Feast.CodeAnalysis.Scripting
{
    ...
    /// </summary>
    [Literal("Feast.CodeAnalysis.Scripting.Generates.ScriptBuilder")]
    internal sealed partial class ScriptBuilder
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            using (var pdbStreamOpt = emitDebugInformation ? new MemoryStream() : null)
            {
                ...
                foreach (var referencedAssembly in (IEnumerable)GetReferencedAssemblies.Value(
                    GetBoundReferenceManager.Value(compilation)))
                {
                    var path = (Key(referencedAssembly) as PortableExecutableReference)?.FilePath;
                    if (path != null)
                    {
                        ...
                        _assemblyLoader.RegisterDependency(
                            Identity.Value(Value(referencedAssembly)) as AssemblyIdentity, path);
                    }
                }
                ...
                var assembly          = LoadAssemblyFromStream.Value(_assemblyLoader, peStream, pdbStreamOpt) as Assembly;
                var runtimeEntryPoint = GetEntryPointRuntimeMethod(entryPoint, assembly);

                return runtimeEntryPoint.CreateDelegate(typeof(Func<object[], Task<T>>)) as Func<object[], Task<T>>;
            }
        }

        // internal for testing
        internal static EmitOptions GetEmitOptions(bool emitDebugInformation)
            => emitDebugInformation ? s_EmitOptionsWithDebuggingInformation : EmitOptionsDefault.Value as EmitOptions;
        ...
        {
            string entryPointTypeName = BuildQualifiedName.Value(entryPoint.ContainingNamespace.MetadataName, entryPoint.ContainingType.MetadataName) as string;
...
[Literal("Feast.CodeAnalysis.Scripting.Generates.ImmutableArrayExtensions")]
internal static class ImmutableArrayExtensions
{
    public static ImmutableArray<Diagnostic> AsImmutable(this IEnumerable<Diagnostic> diagnostics) => new ImmutableArray<Diagnostic>().AddRange(diagnostics);
}